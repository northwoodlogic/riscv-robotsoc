...
main(uint8_t id)
{
    ...
    while (1) {
        ...
        if (rsio->ppmi[PPMI_THR].sts) {
             ...
             * conversion only gets to about 75% pulse width modulation on the
             * throttle drive signals due to the DX2E transmitter.
             *
             * The full ppmi measurement range is 8 bits, with forward &
             * reverse each having 7 bits. The pulse width modulator
             * implements 8 bits. Left-shift-by-1 scales the 7 bit magnitude
             * back to 8 bits.
             */
            ...
            /* Forward */
                ...
                rsio->pwmo[PWMO_REV].val = 0;
                rsio->pwmo[PWMO_FWD].val = (v - 128) << 1;
            /* Reverse */
            } else {
                rsio->pwmo[PWMO_FWD].val = 0;
                rsio->pwmo[PWMO_REV].val = (128 - v) << 1;
            }
        }
         ...
         * ~14373 per second | -O2 and link time optimization
         *
